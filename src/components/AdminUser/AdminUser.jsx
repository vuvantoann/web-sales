...
function AdminUser () {
    ...
    const columns = [
      ...
      const dataTable = users?.response?.data?.length > 0 && users?.response?.data?.map((user) => {
    ...
    useEffect(()=> {
        if(isSuccessUpdated && dataUpdated?.response?.status === 'OK') {
            message.success()
            handleCloseDrawer()
    ...
    useEffect(()=> {
        if(isSuccessDelected && dataDeleted?.response?.status === 'OK') {
            message.success()
            handleCancelDelete()
    ...
    useEffect(()=> {
      if(isSuccessDelectedMany && dataDeletedMany?.response?.status === 'OK') {
          message.success()
    ...
    return(
        <div>
            ...
            <div style={{marginTop: '20px'}}>
                    <TableComponent handleDelteMany = {handleDeleteManyUser} columns ={columns} isLoading = {isLoadingUsers} data = {dataTable} onRow={(record, rowIndex) => {
                    return {
                        onClick: event => {
            ...
            <DrawerComponent title='Chi tiết tài khoản người dùng' isOpen={isOpenDrawer} onClose={() => setIsOpenDrawer(false)} width="50%">
            ...
            <ModalComponent footer={<>1234</>} title="Xóa tài khoản người dùng" open={isModalOpenDelete} onCancel={handleCancelDelete} onOk={handleDeleteProduct}>
